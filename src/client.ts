...
export interface ClientOptions {
  /**
   * Defaults to process.env['TED_DEMO_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['TED_DEMO_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['TED_DEMO_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Ted Demo API.
 */
export class TedDemo {
  apiKey: string;
   ...
   * API Client for interfacing with the Ted Demo API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['TED_DEMO_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['TED_DEMO_BASE_URL'] ?? https://believe.cjav.dev] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('TED_DEMO_BASE_URL'),
    apiKey = readEnv('TED_DEMO_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.TedDemoError(
        "The TED_DEMO_API_KEY environment variable is missing or empty; either provide it, or instantiate the TedDemo client with an apiKey option, like new TedDemo({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://believe.cjav.dev`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? TedDemo.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('TED_DEMO_LOG'), "process.env['TED_DEMO_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('TED_DEMO_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  static TedDemoError = Errors.TedDemoError;
...
TedDemo.Believe = Believe;
TedDemo.Biscuits = Biscuits;
TedDemo.Characters = Characters;
TedDemo.Coaching = Coaching;
TedDemo.Conflicts = Conflicts;
TedDemo.Episodes = Episodes;
TedDemo.Health = Health;
TedDemo.Matches = Matches;
TedDemo.PepTalk = PepTalk;
TedDemo.Press = Press;
TedDemo.Quotes = Quotes;
TedDemo.Reframe = Reframe;
TedDemo.Stream = Stream;
TedDemo.TeamMembers = TeamMembers;
TedDemo.Teams = Teams;
TedDemo.Version = Version;
TedDemo.Webhooks = Webhooks;
TedDemo.Ws = Ws;
