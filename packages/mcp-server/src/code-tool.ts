...
import { ClientOptions } from 'ted-demo';

const prompt = `Runs JavaScript code to interact with the Ted Demo API.
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    headers: {
      ...
      client_envs: JSON.stringify({
        TED_DEMO_API_KEY: requireValue(
          readEnv('TED_DEMO_API_KEY') ?? client.apiKey,
          'set TED_DEMO_API_KEY environment variable or provide apiKey client option',
        ),
        TED_DEMO_BASE_URL: readEnv('TED_DEMO_BASE_URL') ?? client.baseURL ?? undefined,
      }),
    },
    body: JSON.stringify({
      project_name: 'ted-demo',
      code,
      intent,
      client_opts: {},
